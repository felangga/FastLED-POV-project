...
#define DATA_PIN  MOSI  //13 // (8,15),( 4,2) (6,12) (7,13) 
#define CLOCK_PIN  SCK  //
CRGB leds[NUM_LEDS];



String SSID = "lau"; //add 15-29
String SSIDPASS = "lau121314151617"; //add 30-44
...
void parse_webSocket(String income, byte num) {
  ...
  //'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    ...
    lineInterval += 100;   // Set Wifi Mode MyMode  [-Standalone-]
    webSocket.sendTXT(client_num, "RPM$" + String(lineInterval));
  }//EEPROM.write(1, value[0]);EEPROM.commit(); }
  else if (cammand == "SHN") {
    lineInterval -= 10;   // myHostname=value; // Set Host Name myHostname   [-Standalone-]
    webSocket.sendTXT(client_num, "RPM$" + String(lineInterval));
  }
    ...
    lineInterval += 1;   //AP_SSID=value; // Set AP Name AP_SSID   [-Standalone-]
    webSocket.sendTXT(client_num, "RPM$" + String(lineInterval));
  }
  else if (cammand == "APP") {
    lineInterval -= 1;   // AP_SSIDPASS=value; // Set AP Password   AP_SSIDPASS      [-Standalone-]
    webSocket.sendTXT(client_num, "RPM$" + String(lineInterval));
  }
  else if (cammand == "ESI") {
    EEPROM.write(2, value[0]);  //  [-Standalone-]
    EEPROM.commit();
    (value[0] == '1') ? STATIC = true : STATIC = false;
  }
  else if (cammand == "SIP") {
    lineInterval -= 100;   //Server_IP=value;// Set Server IP Server_IP     [-Standalone-]
    webSocket.sendTXT(client_num, "RPM$" + String(lineInterval));
  }

  else if (cammand == "STI") {
    lineInterval += 10;   // Static_IP=value;// Set Static_IP  81-82   [-Standalone-]
    webSocket.sendTXT(client_num, "RPM$" + String(lineInterval));
  }
  ...
  else if (cammand == "CSP") if (IMAGE_NUM) {
      for (byte i = 0; i < IMAGE_NUM; i++) copy_file_from_SPIFF_to_Raw_Flash("/SHOW/" + IMAGES[i], i);    //  [-Standalone-]
      Store_address();
    }
...
void handleConfig() {
  if (!server.authenticate(preSetUser.c_str(), preSetPass.c_str())) return server.requestAuthentication();
  ACCESS = true;
  send_config_html();
}
void handleOther() {
...
void print_chip_info(void) {
  ...
  preSetPass = String(ESP.getChipId(), HEX);
  preSetPass.toUpperCase();
  Serial.println(preSetUser);
  Serial.println(preSetPass);
